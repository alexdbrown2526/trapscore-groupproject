...
router.get('/', (req, res) => {
  ...
  let dataToSend = {};
  try {
    promises.push(pool.query(`SELECT * from "squad_trap"
    ...
    promises.push(pool.query(`SELECT json_agg(row_to_json(tra)) as traps
                              FROM (
                                SELECT tr."id", tr."name",
                                (SELECT json_agg(sq_tr)
                                FROM (
                                  ...
                                  AND tr."competition_id" = ${req.user.competition_id}
